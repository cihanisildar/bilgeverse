...
import { Save, Send, ArrowLeft, CheckCircle, XCircle, MinusCircle, Eye, Award } from "lucide-react";
...
export default function AdminEditWeeklyReportPage() {
  ...
  const calculateAttendanceScore = () => {
    ...
    const fixedValues = Object.values(formData.fixedCriteria).filter(v => v);
    totalCriteria += fixedValues.length;
    completedCriteria += fixedValues.filter(v => v === "YAPILDI").length;

    // Count variable criteria
    const variableValues = Object.values(formData.variableCriteria).filter(v => v);
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const payload = {
        ...formData,
        submissionDate: formData.status === "SUBMITTED" ? new Date().toISOString() : null
      };
          ...
          <div className="flex-1">
            <h1 className="text-3xl font-bold text-gray-800">
              <span className="bg-gradient-to-r from-indigo-600 to-purple-600 bg-clip-text text-transparent">
                Rapor Düzenle - {report.weekNumber}. Hafta
              </span>
            </h1>
            <p className="text-gray-600">
              {report.period.name} dönemi - {report.user.firstName && report.user.lastName
        ...
        <div className="max-w-6xl mx-auto grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="lg:col-span-2 space-y-6">
            ...
            <Card className="border-0 shadow-md">
              ...
              <CardContent>
                <Textarea
                  placeholder={`${report.weekNumber}. hafta için açıklamalar...`}
          ...
          <div>
            <Card className="border-0 shadow-md sticky top-6">
              <CardHeader>
                <CardTitle>Admin Kontrolleri</CardTitle>
              </CardHeader>
              <CardContent className="space-y-4">
                <div>
                  <Label htmlFor="status">Rapor Durumu</Label>
                ...
                <div>
                  <Label htmlFor="points-awarded">Verilen Puan</Label>
                  <Input
                    id="points-awarded"
                    type="number"
                    value={formData.pointsAwarded}
                    onChange={(e) => setFormData(prev => ({ ...prev, pointsAwarded: Number(e.target.value) }))}
                    min="0"
                    max="100"
                  />
                  <p className="text-xs text-gray-500 mt-1">
                    Önerilen: {Math.round(attendanceScore / 10)} (Katılım: %{attendanceScore})
                  </p>
                </div>
                ...
                <div className="pt-4 border-t">
                  <div className="flex items-center space-x-2 mb-2">
                    ...
                    <span className="text-sm font-medium">Katılım Skoru</span>
                    <Badge variant="outline">%{attendanceScore}</Badge>
                  </div>
                  <p className="text-xs text-gray-500">
                    Otomatik hesaplanan katılım oranı
                  </p>
                </div>
