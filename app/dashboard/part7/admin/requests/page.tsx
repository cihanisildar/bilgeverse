...
export default function AdminRequests() {
  ...
  const handleStatusChange = (status: string) => {
    ...
    router.push(`/dashboard/part7/admin/requests?${params.toString()}`);
  };
  ...
  const processRequest = async (requestId: string, status: 'approved' | 'rejected', note?: string) => {
    try {
      ...
      setRequests(prev =>
        prev.map(req =>
          req.id === requestId
            ? { ...req, status: status === 'approved' ? RequestStatus.APPROVED : RequestStatus.REJECTED }
            : req
        )
      );
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-8 gap-4">
        <div>
          <h1 className="text-3xl font-bold text-gray-900 flex items-center">
            ...
            {pendingCount > 0 && (
              <span className="ml-3 px-2.5 py-1 bg-amber-100 text-amber-800 text-sm font-semibold rounded-full">
                {pendingCount} bekliyor
              </span>
            )}
          </h1>
          <p className="mt-2 text-sm text-gray-600">
            Tüm öğrencilerin ürün taleplerini buradan yönetin ve onaylayın.
          </p>
        </div>
        ...
        <div className="bg-white shadow-sm rounded-lg p-3 flex flex-col sm:flex-row gap-2 items-center">
          ...
          <div className="flex flex-wrap gap-2">
            {[
              { value: 'all', label: 'Tümü', activeClass: 'bg-indigo-600 text-white shadow-md ring-2 ring-indigo-300' },
              { value: 'pending', label: 'Beklemede', activeClass: 'bg-amber-500 text-white shadow-md ring-2 ring-amber-200' },
              { value: 'approved', label: 'Onaylanan', activeClass: 'bg-emerald-600 text-white shadow-md ring-2 ring-emerald-200' },
              { value: 'rejected', label: 'Reddedilen', activeClass: 'bg-rose-600 text-white shadow-md ring-2 ring-rose-200' },
            ].map(({ value, label, activeClass }) => (
              <button
                key={value}
                className={`px-4 py-1.5 rounded-full text-sm font-medium transition-all duration-200 flex items-center gap-1.5
                  ${statusFilter.toLowerCase() === value ? activeClass : 'bg-gray-100 text-gray-700 hover:bg-gray-200'}`}
      ...
      {requests.length === 0 ? (
        <div className="bg-white shadow-sm rounded-lg p-12 text-center">
          <div className="flex flex-col items-center">
            ...
            <h3 className="text-lg font-medium text-gray-900 mb-1">Talep bulunamadı</h3>
            <p className="text-gray-500 max-w-md">
              Seçilen filtreye uygun talep bulunmamaktadır.
            </p>
          </div>
        </div>
      ) : (
        <div className="grid grid-cols-1 gap-4">
          {requests.map((request) => (
            <div key={request.id} className="bg-white rounded-lg shadow-sm border border-gray-200 p-4">
                    ...
                    <div className="text-sm text-gray-500">
                      ...
                      {request.tutor && (
                        <span className="ml-2 text-gray-400">
                          · Rehber: {request.tutor.firstName} {request.tutor.lastName}
                        </span>
                      )}
                    </div>
                  </div>
              ...
              <div className="mb-4">
                <div className="font-medium text-gray-900 mb-1">{request.item.name}</div>
                <div className="text-sm text-gray-500">{request.item.description}</div>
              </div>
          ...
          >
            <div className="flex justify-between items-center mb-5">
              <div className="flex items-center">
                ...
                <h3 className="text-lg font-medium text-gray-900">Talebi Reddet</h3>
              </div>
            ...
            <p className="text-sm text-gray-600 mb-5">
              Lütfen bu talebi reddetme nedeninizi belirtin.
            </p>
            ...
            <div className="mb-5">
              ...
              <textarea
                id="note"
                rows={3}
                className="shadow-sm focus:ring-indigo-500 focus:border-indigo-500 block w-full sm:text-sm border border-gray-300 rounded-lg px-3 py-2 placeholder-gray-400"
