...
export default function AthleteList() {
    ...
    const upsertAthleteMutation = useUpsertAthleteProfile();
    const registerAthleteMutation = useRegisterNewAthlete();
    const deleteAthleteMutation = useDeleteAthleteProfile();
    ...
    const handleDeleteAthlete = async () => {
        ...
        const result = await deleteAthleteMutation.mutateAsync(deletingAthleteId);
        if (!result.error) {
    ...
    const handleEditAthlete = (athlete: any) => {
        // Use a timeout to ensure the DropdownMenu closes completely before opening the Dialog
        // This prevents Radix focus trap/overlay issues
        setTimeout(() => {
        ...
        <div className="space-y-6">
                                ...
                                <div className="flex items-center gap-4">
                                    ...
                                    <DropdownMenu modal={false}>
                                        ...
                                        <DropdownMenuContent align="end">
                                            <DropdownMenuItem onSelect={() => handleEditAthlete(athlete)}>
                                                <Pencil className="h-4 w-4 mr-2" /> Düzenle
                                            </DropdownMenuItem>
                                            <DropdownMenuItem onSelect={() => setDeletingAthleteId(athlete.user.id)} className="text-red-600">
            ...
            <Dialog open={isAdding} onOpenChange={(open) => { setIsAdding(open); if (!open) resetAddForm(); }}>
                <DialogContent className="sm:max-w-[550px] max-h-[90vh] overflow-y-auto">
                    <DialogHeader>
                        <DialogTitle>{editingAthlete ? 'Sporcu Bilgilerini Düzenle' : 'Sporcu Kaydı'}</DialogTitle>
                        <DialogDescription>
                            {editingAthlete ? 'Sporcu profil bilgilerini ve branşlarını güncelleyin.' : 'Sistemdeki bir öğrenciyi seçin veya tamamen yeni bir sporcu kaydı oluşturun.'}
                        </DialogDescription>
                    </DialogHeader>
                    ...
                    {editingAthlete && (
                        <div className="p-4 bg-indigo-50 rounded-2xl flex justify-between items-center border border-indigo-100 mb-4">
                            <div className="flex items-center gap-3">
                                <Avatar className="h-12 w-12 border-2 border-white shadow-sm">
                                    <AvatarImage src={editingAthlete.user.avatarUrl ?? undefined} />
                                    <AvatarFallback className="bg-indigo-200 text-indigo-700 font-bold">
                                        {editingAthlete.user.firstName?.[0]}{editingAthlete.user.lastName?.[0]}
                                    </AvatarFallback>
                                </Avatar>
                                <div>
                                    <span className="font-bold text-indigo-900 block">{editingAthlete.user.firstName} {editingAthlete.user.lastName}</span>
                                    <span className="text-xs text-indigo-600">@{editingAthlete.user.username}</span>
                                </div>
                            </div>
                        </div>
                    )}
                    ...
                    <DialogFooter className="mt-6">
                        ...
                        <Button
                            ...
                            disabled={saving || (editingAthlete ? (selectedBranches.length === 0) : (registerTab === 'existing' ? (!selectedStudent || selectedBranches.length === 0) : (!newAthlete.username || !newAthlete.firstName || !newAthlete.lastName || selectedBranches.length === 0)))}
                        >
                            {saving ? 'Kaydediliyor...' : (editingAthlete ? 'Değişiklikleri Kaydet' : 'Sporcuyu Kaydet')}
                        </Button>
                    </DialogFooter>
                </DialogContent>
            </Dialog>

            <AlertDialog open={!!deletingAthleteId} onOpenChange={(open) => !open && setDeletingAthleteId(null)}>
                <AlertDialogContent>
                    <AlertDialogHeader>
                        <AlertDialogTitle>Emin misiniz?</AlertDialogTitle>
                        <AlertDialogDescription>
                            Bu sporcu profilini silmek istediğinizden emin misiniz? Bu işlem kullanıcının sporcu rolünü ve branş bağlantılarını kaldıracaktır. Kullanıcı sistemden tamamen silinmeyecektir.
                        </AlertDialogDescription>
                    </AlertDialogHeader>
                    <AlertDialogFooter>
                        <AlertDialogCancel>İptal</AlertDialogCancel>
                        <AlertDialogAction onClick={handleDeleteAthlete} className="bg-red-600 text-white hover:bg-red-700">Sil</AlertDialogAction>
                    </AlertDialogFooter>
                </AlertDialogContent>
            </AlertDialog>
        </div>
